...
def start_game_singleplayer(mode=False):
    ...
    while game.running:
        ...
        for event in pygame.event.get():
            ...
            # Key pressed
                ...
                # Skip song on m key
                    ...
                    game.bøffel_image_index = (game.bøffel_image_index + 1) % (len(bøffel_images)-1)
                # Pause song
                elif event.key == pygame.K_b:
                    game.view.menu.music_paused = not game.view.menu.music_paused
                    pygame.mixer.music.pause() if game.view.menu.music_paused else pygame.mixer.music.unpause()
        ...
        if not (game.over or game.paused):
            ...
            if game.bøffel_mode:
                show_bøffel = True
                next_y, held_y, score_y, speed_y, name_y, bøffel_y = 11.5,17,10,50,80,115
            else: 
                next_y, held_y, score_y, speed_y, name_y, bøffel_y = 10,16,10,80,120,200
                show_bøffel = False
            # Redraw game, reset necessary values and lists
            game.update_singleplayer(next_y, held_y, score_y, speed_y, name_y, bøffel_y, show_bøffel)
        pygame.display.update()
        game.clock.tick(60) 
