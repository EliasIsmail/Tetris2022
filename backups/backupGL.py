...
class Game():
    def __init__(self, multiplayer=False, bøffel_mode = False):
        ...
        self.speed_value = 1
        self.image_threshold = 200
        self.video_threshold = 2000
        self.bøffel_mode = bøffel_mode
        self.bøffel_image_index = randint(0,76)
        self.bøffel_video_index = randint(0,19)
        self.view = GameView()
    ...
    def move_piece(self, player):
        ...
        player.remove_piece_from_lines(self.view.board_color)
        for n in range(4):
    ...
    def setLineEmpty(self, player, y):
        for x in range(BOARD_WIDTH):
            player.lines[y-player.ymin][x] = self.view.board_color
    ...
    def is_legal_move(self, player, ls):
        ...
        boolean = player.is_line_free(self.view.board_color, ls)
        return boolean
    ...
    def draw_silhouette(self, player):
        ...
        for n in range (4):
            [x,y] = player.s_list[n]
            (fill_color, rect_color) = (color, self.view.board_color) if [x,y] in player.cp_list[:4] else (self.view.board_color, color)
    ...
    def pop_line_singleplayer(self, player, n):
        ...
        player.score_value += int (line_bonus * player.speed_value * 0.2)
        for i in range(n, 0, -1):
            player.lines[i] = player.lines[i-1].copy()
        self.check_level(player)

    def check_level(self, player):
        if player.score_value > 0 and player.speed_value < max_speed:
            if player.score_value > self.image_threshold:
                if self.bøffel_mode:
                    self.bøffel_image_index = (self.bøffel_image_index + randint(1,10)) % (len(bøffel_images)-1)
                # pygame.time.set_timer(player.event,0)
                self.speed_value += 1
                # player.time -= 30
                self.fall_frequency -= 0.02
                self.image_threshold = int (self.image_threshold * 1.5)
                # pygame.time.set_timer(player.event, player.time)
            if player.score_value > self.video_threshold and self.bøffel_mode:
                self.video_threshold = int (self.video_threshold * 2)
                pygame.mixer.music.pause()
                self.paused = True
                self.view.show_paused(single_player.xmin*block_size+50, 350)
                response = play_video(self.bøffel_video_index)
                self.bøffel_video_index = (self.bøffel_video_index + randint(1,5)) % (len(bøffel_videos)-1)
    ...
    def check_score(self, score):
        self.leaderboard.append((single_player.name, score))
        self.leaderboard = sorted(self.leaderboard, key=itemgetter(1), reverse=True)[:20]
        with open('leaderboard.txt', 'wb') as f:
            pickle.dump(self.leaderboard, f)
        if (single_player.name,score) in self.leaderboard: return True
        # for (n, s) in self.leaderboard:
        #     if (single_player.name == n and score == s): return True
        else: return False

    def update_multiplayer(self, players):
        for player in players:
            self.view.clear_lines(player) ; self.view.draw_lines(player) ; self.draw_silhouette(player) ; \
